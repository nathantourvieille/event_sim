...
parser = argparse.ArgumentParser() # Load IsaacLab's default arguments 
...
# 1. Update the Observer Drone Config
    ...
    observer_cfg.actuators = {"dummy_actuator": ImplicitActuatorCfg(joint_names_expr=[".*"], stiffness=0.0, damping=0.0)}  # Prevents the missing attribute crash
    ...
    target_cfg.actuators = {"dummy_actuator": ImplicitActuatorCfg(joint_names_expr=[".*"], stiffness=0.0, damping=0.0)}  # Prevents the missing attribute crash
    ...
    # env = ManagerBasedRLEnv(cfg=dvs_cam)
...
# --- ONE-TIME SEMANTIC ID LOOKUP ---
    ...
    while simulation_app.is_running():
        try:
            ...
            # --- DEFENSIVE CHECK: WAIT FOR CAMERA TO WARM UP ---
            if cam_data.output is None or "semantic_segmentation" not in cam_data.output or cam_data.output["semantic_segmentation"] is None:
                print(f"[INFO] Frame {frame_count}: Waiting for camera sensors to warm up...")
                # Advance time and skip processing this frame
                prev_snap = dvs.snapshot()
                t_prev += dt
                frame_count += 1
                continue
...
# --- 3. DYNAMIC SEMANTIC ID LOOKUP ---
            ...
            if enemy_id is not None:
                ...
                # Fallback (empty mask) if enemy not visible yet
                # Ensure the empty mask is 2D (Height, Width) even if the tensor has a color channel
                mask_shape = seg_tensor.shape[:2] if len(seg_tensor.shape) > 2 else seg_tensor.shape
